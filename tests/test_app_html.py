...
class AppHtmlTests(unittest.TestCase):
    ...
    def test_segment_inspector_css_keeps_actions_and_media_balanced(self):
        ...
        self.assertIn(".storyboard-card-active::before { content: \"\";", html)
        self.assertIn("background: conic-gradient(from 0deg, var(--studio-accent) 0 23%, transparent 23% 27%, var(--studio-pink) 27% 50%, transparent 50% 54%, var(--studio-accent) 54% 77%, transparent 77% 81%, var(--studio-pink) 81% 100%);", html)
