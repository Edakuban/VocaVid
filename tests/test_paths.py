...
from musicvideogen.paths import resolve_storage_path, storage_relative_path
...
class PathStorageTests(unittest.TestCase):
    ...
    def test_storage_relative_path_converts_legacy_musicvideogen_absolute_paths(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            app_root = Path(directory) / ".musicvideogen"
            legacy = r"D:\old\MusicvideoGen\.musicvideogen\outputs\demo\clip.mp4"
