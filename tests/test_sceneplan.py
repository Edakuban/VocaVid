...
class ScenePlanTests(unittest.TestCase):
    def test_make_sceneplan_prompt_requests_continuous_indexed_video_plan_with_project_context(self):
        ...
        self.assertIn("At least 50% of all segments must focus on something other than the main character.", prompt)
        ...
        self.assertIn("must feature the reference performer", prompt)
        self.assertIn("more than 1 out of 4", prompt)
    ...
    def test_make_sceneplan_concept_prompt_requests_video_bible_with_chorus_escalation(self):
        ...
        self.assertIn("Every chorus/refrain segment with uses_reference=True", prompt)
        self.assertIn("more than 1 out of 4 non-reference segments", prompt)
