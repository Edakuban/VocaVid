...
class PromptgenTests(unittest.TestCase):
    ...
    def test_make_videoprompt_prompt_contains_controlled_performance_rules(self):
        ...
        self.assertIn("Write one continuous visual moment in one shot", prompt)
        self.assertIn("Lip-sync must be clear, natural", prompt)
        self.assertIn("Rhythmic lighting may pulse, flicker, or shift color with the music", prompt)
